...
# make connection to MS Access Database
conn_str = (
...
cursor = cnxn.cursor()
query = f'SELECT * FROM [РД]'
new_df = pd.read_sql(query, cnxn)
...
def new_threading(df_1, df_2):
    ...
    def finding_empty_rows(df):
        if df[col] in ['nan', 'None'] or pd.isna(df[col]):
            return ''
        else:
            return df[col]
    ...
    tmp_df = df_1[df_1['_merge'] == 'both'][columns.mdf1_columns]
    tmp_df.columns = columns.new_columns

    
    # generate result dataframe, with remove document firstly
    print('Generating result dataframe, with remove document firstly')
    changed_df = df_2[df_2['_merge'] == 'left_only'][columns.new_columns]
    tmp_df = pd.concat([changed_df, tmp_df])

    #  generate result dataframe 
    print('Generating result dataframe ')
    changed_df = df_2[df_2['_merge'] == 'both'][columns.mdf2_columns]
    changed_df.columns = columns.new_columns
    ...
    for col in changed_df.columns:
        changed_df[col] = changed_df.apply(finding_empty_rows, axis = 1)
    ...
    # saving the result-file in Database
    cursor.execute(f"DROP TABLE [РД]")
    cnxn.commit()
    ...
    for row in changed_df.itertuples(index=False):
        insert_query = f'''INSERT INTO [РД] ([Система], 
                                             [Наименование], 
                                             [Код], 
                                             [Тип], 
                                             [Пакет], 
...
# fucntion for changing WBS
def changing_wbs(row):
    split_row = row.split()
    if len(split_row) > 1:
...
new_df.columns = columns.base_columns

#  Clearing dataframes
print('Clearing dataframes')
base_df = base_df.dropna(subset=['Коды работ по выпуску РД'])
base_df['Разработчики РД (актуальные)'] = base_df.apply(changing_developer, axis = 1)
base_df = base_df[~base_df['Коды работ по выпуску РД'].str.contains('\.C\.', regex=False)]
base_df['Объект'] = base_df['Объект'].apply(lambda row: row[ : row.find(' ')])
base_df['WBS'] = base_df['WBS'].apply(changing_wbs)
...
m_df_2 = (tmp_df.merge(base_df_copy,
                           ...
                           suffixes=['', '_new'],
                           indicator=True))
m_df_1['Статус текущей ревизии_new'] = m_df_1.apply(changing_status, axis = 1)
m_df_2['Статус текущей ревизии_new'] = m_df_2.apply(changing_status, axis = 1)
...
#changing dates
for col in columns.convert_columns:
    dayfirst = False
    if 'new' in col:
        if flag:
            ...
            m_df_1[col] = m_df_1[col].apply(converting_date)
            m_df_2[col] = m_df_2[col].apply(converting_date)
    else:
        dayfirst = True
        m_df_1[col] = m_df_1[col].apply(converting_date)
        m_df_2[col] = m_df_2[col].apply(converting_date)
...
# preparing log-file
m_df = pd.concat([m_df_1[columns.changed_cols], m_df_2[columns.changed_cols]])
m_df = m_df.reset_index()[columns.changed_cols]
max_len_row = [max(m_df[row].apply(lambda x: len(str(x)) if x else 0)) for row in m_df.columns]
max_len_name = [len(row) for row in columns.changed_cols]
...
with open(f'{output_filename}.txt', 'w') as log_file:
    ...
    for column, col_len in zip(columns.changed_cols, max_len):
        file_write += f"{column:<{col_len}}|"
    log_file.write(file_write)
    log_file.write('\n')
    for index, row in m_df.iterrows():
        column_value = ''
        for i in range(len(columns.changed_cols)):
            column_value += f"{str(row[columns.changed_cols[i]]) if row[columns.changed_cols[i]] else '-':<{max_len[i]}}|"
